...
const StationeryOverview = ({
	...
	const breadcrumbs = [
		{ id: 1, name: `${t('stationeryid:breadcrumb_home')}`, href: '/' },
		{
			id: 2,
			name: `${t('stationeryid:breadcrumb_books')}`,
			href: '/stationery',
		},
	];
				...
				<nav aria-label="Breadcrumb">
					...
					>
						...
						<li className="text-sm">
							<Link href={`/stationery/${stationery.id}`}>
								<a
									rel="noopener noreferrer"
									aria-current="page"
						...
						<div className="mt-6">
							<div className="flex items-center">
								...
								<Link href={reviews.href}>
									...
									>
										{reviews.totalCount} {t('stationeryid:reviews')}
									</a>
								</Link>
							</div>
						</div>
...
StationeryOverview.getInitialProps = async (context, client, currentUser) => {
	if (currentUser) {
		const { stationeryId } = context.query;
		const { data } = await client.get(`/api/stationery/${stationeryId}`);
		const response = await client.get(`/api/cart/${stationeryId}`);

		const inCart = _.isEmpty(response.data) ? false : true;

		return { stationery: data, status: inCart };
	}

	const { stationeryId } = context.query;
	const { data } = await client.get(`/api/stationery/${stationeryId}`);
	return { stationery: data };
};

export default StationeryOverview;
